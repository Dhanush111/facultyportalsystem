...
if (mysqli_num_rows($result) > 0) {
  ...
  echo '<div class="card">';
  echo '<h5 class="card-header">Seminars</h5>';
  ...
  while($row = mysqli_fetch_assoc($result)) {
    ...
    echo '<td>'.$row["seminar_title"].'</td>';
    echo '<td>'.$row["seminar_date"].'</td>';
    echo '<td>'.$row["seminar_location"].'</td>';
    echo '</tr>';
    $count++;
  }
  echo '</tbody>';
  ...
  echo "No seminars found in the database.";
}
